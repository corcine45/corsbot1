...
def _load_settings() -> Settings:
    ...
    missing = [
        f"{var} ({desc})" for var, desc in _REQUIRED.items() if not os.getenv(var)
    ]
    if missing:
        for m in missing:
    ...
    if bad:
        ...
        log.error(
            "Update your environment variables (e.g. .env or host env) with real secrets and restart."
        )
        raise SystemExit(1)
    ...
    return Settings(
        ...
        tenor_api_key=os.getenv(
            "TENOR_API_KEY", "AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCyk"
        ),
    )
...
QUICK_REPLIES = {
    ...
    frozenset(["gg", "good game", "ggs"]): ["gg!", "ggs", "well played", "gg ez"],
    frozenset(
        [
            "pog",
            "poggers",
